...
def status(x):
    return pd.Series([x.count(), length - x.count(), len(credit.groupby(by=x)), x.max() - x.min(),
                      x.quantile(.75) -
                      x.quantile(.25), x.mode()[0], format(x.var(), '.3f'),
...
describe_tb.to_excel('output.xlsx', index=False)
...
def credit_plot(column, i):
    ...
    ax.bar(2, not_y[2], bottom=not_y[1], color='b',
           label='否', width=0.3)  # 绘制柱形图
...
def risk_plot(column1, column2, xlabel_list=[], ylabel_list=[]):
    ...
    co = list(map(lambda x: color(tuple(x)),
              ncolors(len(ylabel_list))))  # 指定数量的颜色
    ...
    for i in sorted(x_data.unique()):
        ...
        if part[0] == 0:
            for j in part:
                ...
                ax.bar(i, exp1, bottom=exp, width=0.5,
                       color=co[j], label=ylabel_list[j])
                exp += exp1
        else:
            for j in part:
                exp1 = y_data.value_counts()[j] / len(y_data)
                ax.bar(i, exp1, bottom=exp, width=0.5,
                       color=co[j-1], label=ylabel_list[j-1])
                exp += exp1
...
# 图例去重
    ...
    plt.legend(by_label.values(), by_label.keys(), loc=[
               1.01, 0], fontsize=10, title=column2+'（万元）')
